...
Run once:
    python data/alx_1D/build_templates.py            # build from the default sources
    python data/alx_1D/build_templates.py --fetch    # pull from Drive first, then build
    python data/alx_1D/build_templates.py --src a.pkl:synthetic b.json:real
    python data/alx_1D/build_templates.py --max-natm 128   # pin the cap explicitly
...
def build(specs=None, max_natm=None, collect_max=COLLECT_MAX,
    ...
    if missing:
        raise SystemExit(
            ...
            '\nFetch them from Drive first:  python data/alx_1D/gdrive_fetch.py')
    ...
    # Pass 1: read every source, reduce oversized tubes, dedup, and collect.
    ...
    print(f'\nusing MAX_NATM = {max_natm}  (pre-filter skipped: {too_big}, '
          f'cross-source dupes: {dupes})')
