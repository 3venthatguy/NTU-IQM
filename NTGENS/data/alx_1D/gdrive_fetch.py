...
downloads any that are missing locally, so build_templates.py / build_train_csv.py
...
Usage:
    python data/alx_1D/gdrive_fetch.py                 # fetch all missing files
    python data/alx_1D/gdrive_fetch.py --force         # re-download even if cached
    python data/alx_1D/gdrive_fetch.py --list          # show manifest vs. local state
"""
import argparse
import hashlib
import json
...
def _load_manifest(path=MANIFEST):
    ...
    # Explicit null (or missing) dest_dir -> this data/alx_1D folder.
