...
> **The frame is rebuilt from the current lattice at every step** (`cyl_cfg['track_frame']`, default on). The pinned cell only *emerges* over the trajectory — `l_T` is pure noise (~1 Å) and grows into the template cell as `C0 = √ᾱ` rises — so a frame frozen in absolute template Ångströms sits outside the early cell and collapses every atom onto one azimuth, which the θ-invariant radial terms then lock in. `estimate_template_bounds` therefore also returns `centroid_frac`, carried on the batch as `tube_centroid_frac`, and the band is scaled by the transverse cell scale `s_t` (→ `s_t = 1` at `t=0`, i.e. unchanged behaviour). An optional `apply_angular_spread` term (`ang_cfg`, off by default) spreads atoms in θ, which no other term touches. See [technical-foundations.md](../technical-foundations.md) §5b, and [known-discrepancies.md](../known-discrepancies.md) §8 for why `pin_psi_start` must stay 0.
